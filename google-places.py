...
while loop_true:
    ...
    if len(data['results']) > 0: #If the API returned at least a result
        for index in range(len(data["results"])):
            ...
            details = r.content.decode("utf-8")
            details = json.loads(details)

            try:
                name = details.get('result', {}).get('name', '') #get the name from place details
                address = details.get('result', {}).get('formatted_address', '') #get the formatted address from place details
                phone_number = details.get('result', {}).get('international_phone_number', '') #get the international phone number from place details
                website = details.get('result', {}).get('website', '') #get the website from place details
